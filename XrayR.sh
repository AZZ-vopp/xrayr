...
# check root
[[ $EUID -ne 0 ]] && echo -e "${red}sai lầm: ${plain} Tập lệnh này phải được chạy với người dùng root！\n" && exit 1
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/AZZ-vopp/xrayr/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/AZZ-vopp/xrayr/master/install.sh) $version
    if [[ $? == 0 ]]; then
        echo -e "${green}Cập nhật hoàn tất và XrayR đã tự động khởi động lại, vui lòng sử dụng nhật ký XrayR để xem nhật ký đang chạy${plain}"
        exit
    fi
...
config() {
    ...
    case $? in
        0)
            ...
            ;;
        1)
            echo -e "Phát hiện bạn chưa khởi động XrayR hoặc XrayR không thể tự khởi động lại, hãy kiểm tra nhật ký？[Y/n]" && echo
...
uninstall() {
    ...
    echo ""
    echo -e "Quá trình gỡ cài đặt thành công. Nếu bạn muốn xóa tập lệnh này, hãy chạy nó sau khi thoát khỏi tập lệnh ${green}rm /usr/bin/XrayR -f${plain} xóa"
    echo ""
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        echo -e "${green}XrayR đã chạy và không cần khởi động lại. Nếu bạn cần khởi động lại, vui lòng chọn Khởi động lại${plain}"
    else
        systemctl start XrayR
        sleep 2
        check_status
        if [[ $? == 0 ]]; then
            echo -e "${green}XrayR đã bắt đầu thành công, vui lòng sử dụng nhật ký XrayR để xem nhật ký đang chạy${plain}"
        else
            echo -e "${red}XrayR có thể không khởi động được, vui lòng sử dụng nhật ký XrayR để xem thông tin nhật ký sau${plain}"
        fi
    fi
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        ...
        echo -e "${red}Cài đặt XrayR không tự động bắt đầu${plain}"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        ...
        echo -e "${red}XrayR không thể hủy khởi động tự động${plain}"
    fi
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/AZZ-vopp/xrayr/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
        echo -e "${red}Không thể tải xuống tập lệnh, vui lòng kiểm tra xem máy có thể kết nối với Github không${plain}"
        before_show_menu
    else
        chmod +x /usr/bin/XrayR
        echo -e "${green}Tập lệnh nâng cấp đã thành công, vui lòng chạy lại tập lệnh${plain}" && exit 0
    fi
}
...
show_status() {
    ...
    case $? in
        0)
            ...
            ;;
        1)
            echo -e "Trạng thái XrayR: ${yellow}không chạy bạn ơi${plain}"
            show_enable_status
            ;;
        2)
            echo -e "Trạng thái XrayR: ${red}Chưa cài đặt${plain}"
...
show_enable_status() {
    check_enabled
    if [[ $? == 0 ]]; then
        echo -e "Có bắt đầu tự động hay không: ${green}Có${plain}"
    else
        echo -e "Có bắt đầu tự động hay không: ${red}Không${plain}"
    fi
}

show_XrayR_version() {
    echo -n "Phiên bảnXrayR ："
...
show_usage() {
    echo "Cách sử dụng kịch bản quản lý XrayR: "
    echo "------------------------------------------"
    ...
    echo "XrayR enable       - Đặt XrayR tự khởi động khi khởi động"
...
--- XrayR ---
  ${green}0.${plain} Thay đổi cài đặt cấu hình config
————————————————
...
————————————————
  ${green}9.${plain} Đặt XrayR tự khởi động khi khởi động
 ${green}10.${plain} Hủy tự khởi động XrayR
————————————————
 ${green}11.${plain} Cài đặt bằng một cú nhấp chuột bbr (kernel mới nhất)
 ${green}12.${plain} Xem phiên bản XrayR 
 ${green}13.${plain} Nâng cấp kịch bản bảo trì(bảo trì)
 "
 #Các bản cập nhật tiếp theo có thể được thêm vào chuỗi trên
    ...
    case "${num}" in
        ...
        13) update_shell
        ;;
        *) echo -e "${red}Vui lòng nhập số chính xác đi bạn ơi [0-12]${plain}"
        ;;
    esac
}
